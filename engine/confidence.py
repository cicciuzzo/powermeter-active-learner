...
# Minimum evaluated samples before rolling accuracy dominates
BLEND_N_MIN = 25
...
def confidence_blend(
    ...
    """
    Blend entropy-based confidence with rolling accuracy.

    Starts with pure entropy confidence, transitions linearly to rolling
    accuracy as evaluated feedback count approaches BLEND_N_MIN.
    """
    n = tracker.evaluated_count
    alpha = min(n / BLEND_N_MIN, 1.0)
    rolling = tracker.get_rolling_accuracy()
    return (1.0 - alpha) * entropy_confidence + alpha * rolling
